...
export const Link = React.forwardRef<HTMLAnchorElement, LinkProps>(
  (
    ...
    const classes = ['link', skip && 'link--skip', className].filter(Boolean).join(' ');

    // Skip links should use button semantics if no href
    if (skip && !href) {
      return (
        <button ref={ref as any} className={classes} aria-label={ariaLabel} {...(props as any)}>
          {children}
        </button>
      );
    }
